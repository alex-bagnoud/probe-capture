...
write.table(pc3, "1-tax_comparison_v2.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
pdf("2-amoa_length_distribution_v2.pdf", width = 5, height = 5)
print(plot1)
dev.off()
...
write.table(blast.pc2, "3-blast_pc_with_tax_v2.txt", row.names = FALSE, quote = FALSE, sep = "\t")
...
write.table(m2, "4b_bubble_plot_table.txt", row.names = FALSE, quote = FALSE, sep = "\t")
...
pdf("4a-bubble_plot.pdf", height = 12, width = 6)
print(bubble_plot)
dev.off() 

### 4.15) Simplified bubble plot
...
mm.v2 <- mm %>% 
  mutate(
    sample.type = case_when(
      str_detect(variable, "ampH2_") ~ "Amplicon sequencing",
      str_detect(variable, "pcH2_") ~ "Targeted metagenomics",
      str_detect(variable, "mgH3_16") ~ "Shotgun metagenomics"
    ),
    value = if_else(is.na(value), 0, value)
  ) %>% 
  group_by(sample.type, tax, col) %>% 
  summarise(
    value = mean(value)
  ) %>% 
  mutate(
  ...
  mutate(
    ...
    variable = fct_relevel(variable, "Amplicon sequencing", "Targeted metagenomics", "Shotgun metagenomics")
  )


bubble_plot2 <- ggplot(mm.v2,aes(variable,tax)) +
  geom_point(aes(size=value, fill=col),shape=21,color="black") +
  theme(panel.grid.major=element_line(linetype=1,color="grey"),
...
pdf("4c-bubble_plot_simplified.pdf", height = 12, width = 5)
print(bubble_plot2)
dev.off() 
...
m3 <- mm.v2 %>% 
  ...
  select("tax", "Amplicon sequencing", "Targeted metagenomics", "Shotgun metagenomics", "col")


write.table(m3, "4d_bubble_plot_table_simplified.txt", row.names = FALSE, quote = FALSE, sep = "\t")
